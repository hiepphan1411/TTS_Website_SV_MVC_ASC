    ...
    campusData: {
        'Cơ sở chính': {
            buildings: {
                ...
                'Dãy C': {
                    rooms: ['C1.01', 'C2.01', 'C3.12', 'C3.15'],
                },
                'Dãy D': {
                    rooms: ['D1.01', 'D1.02', 'D2.01', 'D2.02'],
                },
            },
        },
        'Cơ sở 2': {
            buildings: {
                'Dãy E': {
                    rooms: ['E1.01', 'E1.02', 'E1.03', 'E2.01', 'E2.02'],
                },
                'Dãy F': {
                    rooms: ['F1.01', 'F1.02', 'F2.01', 'F2.02'],
                },
            },
        },
        'Cơ sở 3': {
...
function populateDropdowns() {
    // Populate các dropdown đơn giản (không phụ thuộc)
    ...
    const campuses = Object.keys(dropdownData.campusData);
    populateSelect('campus', campuses, '-- Chọn cơ sở --');

    // Khởi tạo dropdown Dãy nhà và Phòng rỗng cho filter
    populateSelect('building', [], '-- Chọn dãy nhà --');
    populateSelect('room', [], '-- Chọn phòng --');

    // Populate cho modal - Cơ sở dropdown
    ...
    // Populate Cơ sở cho modal
    populateSelect('modalCoSo', campuses, '-- Chọn cơ sở --');

    // Khởi tạo dropdown Dãy nhà và Phòng rỗng cho modal
    populateSelect('modalDayNha', [], '-- Chọn dãy nhà --');
    populateSelect('modalPhong', [], '-- Chọn phòng --');
    ...
    // Setup cascade dropdowns cho cả filter và modal
    setupFilterCascadeDropdowns();
    setupModalCascadeDropdowns();
}

// Hàm setup cascade cho filter section dropdowns
function setupFilterCascadeDropdowns() {
    const filterCampus = document.getElementById('campus');
    const filterBuilding = document.getElementById('building');
    const filterRoom = document.getElementById('room');

    if (!filterCampus || !filterBuilding || !filterRoom) return;

    // Khi chọn Cơ sở -> Load Dãy nhà
    filterCampus.addEventListener('change', function () {
        const selectedCampus = this.value;

        // Reset các dropdown phụ thuộc
        filterBuilding.innerHTML =
            '<option value="">-- Chọn dãy nhà --</option>';
        filterRoom.innerHTML = '<option value="">-- Chọn phòng --</option>';
        filterBuilding.disabled = !selectedCampus;
        filterRoom.disabled = true;

        if (selectedCampus && dropdownData.campusData[selectedCampus]) {
            const buildings = Object.keys(
                dropdownData.campusData[selectedCampus].buildings,
            );
            populateSelect('building', buildings, '-- Chọn dãy nhà --');
            filterBuilding.disabled = false;
        }
    });

    // Khi chọn Dãy nhà -> Load Phòng
    filterBuilding.addEventListener('change', function () {
        ...
        filterRoom.innerHTML = '<option value="">-- Chọn phòng --</option>';
        filterRoom.disabled = !selectedBuilding;

        if (
            selectedCampus &&
            selectedBuilding &&
            dropdownData.campusData[selectedCampus] &&
            dropdownData.campusData[selectedCampus].buildings[selectedBuilding]
        ) {
            const rooms =
                dropdownData.campusData[selectedCampus].buildings[
                    selectedBuilding
                ].rooms;
            populateSelect('room', rooms, '-- Chọn phòng --');
            filterRoom.disabled = false;
        }
    });

    // Khởi tạo trạng thái disabled
    filterBuilding.disabled = true;
    filterRoom.disabled = true;
}

// Hàm setup cascade cho modal dropdowns
function setupModalCascadeDropdowns() {
    const modalCoSo = document.getElementById('modalCoSo');
    const modalDayNha = document.getElementById('modalDayNha');
    const modalPhong = document.getElementById('modalPhong');

    if (!modalCoSo || !modalDayNha || !modalPhong) return;

    // Khi chọn Cơ sở -> Load Dãy nhà
    modalCoSo.addEventListener('change', function () {
        const selectedCampus = this.value;

        // Reset các dropdown phụ thuộc
        modalDayNha.innerHTML = '<option value="">-- Chọn dãy nhà --</option>';
        modalPhong.innerHTML = '<option value="">-- Chọn phòng --</option>';
        modalDayNha.disabled = !selectedCampus;
        modalPhong.disabled = true;

        if (selectedCampus && dropdownData.campusData[selectedCampus]) {
            const buildings = Object.keys(
                dropdownData.campusData[selectedCampus].buildings,
            );
            populateSelect('modalDayNha', buildings, '-- Chọn dãy nhà --');
            modalDayNha.disabled = false;
        }
    });

    // Khi chọn Dãy nhà -> Load Phòng
    modalDayNha.addEventListener('change', function () {
        const selectedCampus = modalCoSo.value;
        const selectedBuilding = this.value;

        // Reset dropdown phòng
        modalPhong.innerHTML = '<option value="">-- Chọn phòng --</option>';
        modalPhong.disabled = !selectedBuilding;
        ...
        if (
            ...
            populateSelect('modalPhong', rooms, '-- Chọn phòng --');
            modalPhong.disabled = false;
        }
    });

    // Khởi tạo trạng thái disabled
    modalDayNha.disabled = true;
    modalPhong.disabled = true;
}

// Helper function: Lấy danh sách tất cả phòng từ tất cả cơ sở
function getAllRooms() {
    const allRooms = [];
    Object.values(dropdownData.campusData).forEach((campus) => {
        Object.values(campus.buildings).forEach((building) => {
...
// Helper function: Lấy thông tin chi tiết của phòng
function getRoomDetails(roomName) {
    for (const [campusName, campusData] of Object.entries(
        dropdownData.campusData,
    )) {
        for (const [buildingName, buildingData] of Object.entries(
            campusData.buildings,
        )) {
            if (buildingData.rooms.includes(roomName)) {
                return {
                    room: roomName,
