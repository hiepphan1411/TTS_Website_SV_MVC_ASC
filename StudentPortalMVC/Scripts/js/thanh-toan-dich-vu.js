; (function ($) {
    ...
    //Data demo
    var paymentData = [
        { id: 1, type: 'TN', name: 'Học phí tốt nghiệp', credit: null, obligate: true, amount: 1000000 },
        { id: 2, type: '001549', name: 'Kiến trúc và Thiết kế Phần mềm', credit: 4, obligate: false, amount: 3380000 },
        { id: 3, type: '003147', name: 'Công nghệ mới trong phát triển ứng dụng CNTT', credit: 3, obligate: 'Tùy chọn', amount: 2630000 },
        { id: 4, type: '003098', name: 'Thực tập doanh nghiệp', credit: 5, obligate: false, amount: 3750000 },
        { id: 5, type: 'BHYT2026', name: 'Thu bảo hiểm y tế năm 2026', credit: null, obligate: false, amount: 632000 }
    ];

    var paymentData2 = [
        { id: 1, code: 'KT2301', name: 'Thu đoàn phí năm học 2025 - 2026', type: 'Lệ phí', qty: 1, price: 24000 },
        { id: 2, code: 'KT2001', name: 'Thu hội phí năm học 2025 - 2026', type: 'Lệ phí', qty: 2, price: 24000 }
    ];
    ...
    function renderPaymentTable(filterType) {
        ...
        var rows = $.map(data, function (item, index) {
            ...
            return '<tr data-code="' + item.type + '" data-name="' + item.name + '" data-amount="' + item.amount + '">' +
                ...
                '<td class="text-end"><span class="amount">' + item.amount.toLocaleString('vi-VN') + '</span></td>' +
    ...
    function renderPaymentTableForService(filterType) {
        ...
        var data = filterType
            ? $.grep(paymentData2, function (item) { return item.type === filterType; })
            : paymentData2;

        var rows = $.map(data, function (item, index) {
            var typeHtml = item.type === 'Lệ phí'
                ? '<span class="status-badge required">Lệ phí</span>'
                : '<span class="status-badge optional">... phí</span>';
            var amount = item.price * item.qty;
            return '<tr data-code="' + item.code + '" data-name="' + item.name + '" data-amount="' + amount + '">' +
                '<td class="text-center">' + (index + 1) + '</td>' +
                '<td class="ps-2 fw-bold">' + item.code + '</td>' +
                '<td class="ps-2">' + item.name + '</td>' +
                '<td class="text-center">' + typeHtml + '</td>' +
                '<td class="text-center">' + (item.qty || 0) + '</td>' +
                '<td class="text-end"><span class="amount">' + item.price.toLocaleString('vi-VN') + '</span></td>' +
                '<td class="text-end"><span class="amount">' + amount.toLocaleString('vi-VN') + '</span></td>' +
                '<td class="action-cell"><button class="select-btn"><i class="fas fa-arrow-right"></i></button></td>' +
                '</tr>';
        });
    ...
    function updateSelectedItems() {
        ...
        $selectedRows.each(function () {
            ...
            $list.append(
                ...
                '<button class="remove-item-btn" data-code="' + code + '"><i class="fas fa-times"></i></button>' +
                '</div>'
            );
        });
    ...
    function removeSelectedItem(code) {
        var $row = $('tbody tr[data-code="' + code + '"]');
